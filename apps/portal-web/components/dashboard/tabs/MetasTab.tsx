...
import { TrendingUp, Instagram, Camera, ArrowLeft, ArrowRight, Target, Scale } from "lucide-react"
...
import { RunningGoalsCard } from "@/components/dashboard/goals/RunningGoalsCard"
import { useWeightProgress, useProgressPhotos } from '@/hooks/useWeightProgress'
import { useWeightGoals } from '@/hooks/useWeightGoals'
import { MetasTabSkeleton } from '@/components/dashboard/metas/MetasSkeletons'

export function ProgressoTab() {
  const { goal, refetch: refetchGoal } = useWeightGoals()
  const { data: weightData, loading: weightLoading, refetch: refetchWeight } = useWeightProgress()
  const { photos, loading: photosLoading, refetch: refetchPhotos } = useProgressPhotos()
  
  const isLoading = weightLoading || photosLoading
    ...
    <div className="p-6 space-y-6">
      ...
      <Card className="bg-white border-0 shadow-2xl rounded-3xl overflow-hidden">
        <CardContent className="p-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-gray-900 mb-1">Suas Metas</h1>
              <p className="text-gray-600 text-sm">Acompanhe seu progresso em peso e performance na corrida</p>
            </div>
            
            <div className="flex items-center space-x-2">
              <div className="flex items-center space-x-1 text-sm text-gray-600">
                <Target className="h-4 w-4" />
                <span>2 metas ativas</span>
              </div>
            </div>
          </div>
        </CardContent>
      ...
      <Tabs defaultValue="running" className="w-full">
        <TabsList className="grid w-full grid-cols-2 bg-gray-100 rounded-2xl p-1">
          <TabsTrigger value="running" className="rounded-xl flex items-center">
            <Target className="h-4 w-4 mr-2" />
            Metas de Corrida
          </TabsTrigger>
          <TabsTrigger value="weight" className="rounded-xl flex items-center">
        ...
        <TabsContent value="weight" className="space-y-6 mt-6">
          <WeightGoalsContent 
            actualCurrentWeight={actualCurrentWeight}
            actualTargetWeight={actualTargetWeight}
            actualStartWeight={actualStartWeight}
            actualWeightUpdates={actualWeightUpdates}
            actualProgressPhotos={actualProgressPhotos}
            totalWeightPages={totalWeightPages}
            currentWeightPage={currentWeightPage}
            setCurrentWeightPage={setCurrentWeightPage}
            weightItemsPerPage={weightItemsPerPage}
...
// Component for Weight Goals Content
interface WeightGoalsContentProps {
  actualCurrentWeight: number
  actualTargetWeight: number
  actualStartWeight: number
  actualWeightUpdates: any[]
  actualProgressPhotos: any[]
  totalWeightPages: number
  currentWeightPage: number
  setCurrentWeightPage: (page: number) => void
  weightItemsPerPage: number
  handleEditGoal: () => void
  handleAddWeight: () => void
  handleUpdateClick: (update: any) => void
  handlePhotoClick: (index: number) => void
  getUpdateData: (update: any, index: number) => any
}

function WeightGoalsContent({
  actualCurrentWeight,
  actualTargetWeight,
  actualStartWeight,
  actualWeightUpdates,
  actualProgressPhotos,
  totalWeightPages,
  currentWeightPage,
